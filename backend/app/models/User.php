...
final class User
{
    public static function findByEmail(string $email): ?array {
        $s = Database::conn()->prepare('SELECT * FROM users WHERE email = ?');
        $s->execute([$email]);
        $r = $s->fetch();
        return $r ?: null;
    }
    public static function find(int $id): ?array {
        $s = Database::conn()->prepare('SELECT * FROM users WHERE id = ?');
        $s->execute([$id]);
        $r = $s->fetch();
        return $r ?: null;
    }
    public static function create(string $name, string $email, string $passwordHash, string $role = 'user'): int {
        ...
        $s->execute([$name, $email, $passwordHash, $role]);
    ...
    public static function setStatus(int $id, string $status): void {
        $s = Database::conn()->prepare('UPDATE users SET status = ?, updated_at = NOW() WHERE id = ?');
        $s->execute([$status, $id]);
    }
    public static function updatePassword(int $id, string $hash): void {
        $s = Database::conn()->prepare('UPDATE users SET password_hash = ?, updated_at = NOW() WHERE id = ?');
        $s->execute([$hash, $id]);
    }
    public static function updateProfile(int $id, string $name, string $email): void {
        $s = Database::conn()->prepare('UPDATE users SET name = ?, email = ?, updated_at = NOW() WHERE id = ?');
